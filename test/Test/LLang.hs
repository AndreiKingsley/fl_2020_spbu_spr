...
unit_parseL = do
   runParser parseL "{#ROBIT#{#PUSTO#}}" @?= Success "" Seq {statements = [Seq {statements = []}]}
   ...
   runParser parseL "{#ROBIT#{#KOLI#:@USSR@>$CELKOVIY$$NOL$$NOL$:#TADI#{#ZVYAZATI#@USSR@:$CELKOVIY$+$NOL$-$NOL$:}#PO-INOMU#{#PUSTO#}}}" 
	@?= Success "" Seq {statements = [If {cond = BinOp Gt (Ident "USSR") (Num 100), thn = Assign {var = "USSR", expr = BinOp Minus (BinOp Plus (Num 1)(Num 0)) (Num 0)}, els = Seq {statements = []}}]}
   ...
   runParser parseL "{#ROBIT#{#ZVYAZATI#@USSR@:$CELKOVIY$$DEVYATICHOK$$DEVYATICHOK$$CELKOVIY$+-$CELKOVIY$$DEVYATICHOK$$POLUSHKA$$POLUSHKA$:}{#KOLI#:@USSR@>$CELKOVIY$$NOL$$NOL$:#TADI#{#ZVYAZATI#@USSR@:$CELKOVIY$+$NOL$-$NOL$:}#PO-INOMU#{#PUSTO#}}{#PAKUL#:@USSR@:{#NAPISATNABERESTU#:$ZOLOTNICHOK$:}}}" 
	@?= Success "" Seq {statements = [Assign {var = "USSR", expr = (BinOp Plus (Num 1991) (UnaryOp Minus (Num 1922)))}, If {cond = BinOp Gt (Ident "USSR") (Num 100), thn = Assign {var = "USSR", expr = BinOp Minus (BinOp Plus (Num 1)(Num 0)) (Num 0)}, els = Seq {statements = []}}, While {cond = (Ident "USSR"), body = Write {expr = (Num 8)}}]}